...
- **去重归档页**：选文件夹 → **开始预演**（先看清单，不动文件）→ 确认无误再点**一键归档**；做错了直接点**撤销本次归档**原样还原。详见下一节。
...
- **`-k/--keep`** 决定哪张当基准图：`first`=名字最短最靠前（默认）、`largest`=体积最大、`oldest`/`newest`=最早/最新。
  默认规则是"**顶层优先 → 名字短的优先 → 文件名顺序**"，因为副本的名字通常更长（`海边-副本.png`、`照片 (1).jpg`、`xx_备份.png`），
  这样 `海边.png` 会当基准而不是被副本抢走；同级的 `a.png` / `b.png` 仍按文件名顺序取 `a.png`。
...
GUI 的第三个页签 **“去重归档”** 是同一套能力的可视化版本，不习惯命令行就用它：

| 界面元素 | 作用 |
|---|---|
| 图片文件夹 + 浏览 | 选要整理的文件夹 |
| 判重方式 / 阈值 / 保留 / 含子文件夹 | 对应 `--mode` / `-t` / `--keep` / `--no-recursive` |
| **开始预演** | 只扫描并列出清单（组 / 文件 / 说明），**一个文件都不动**；没预演过时"一键归档"是灰的 |
| **一键归档(移动文件)** | 弹确认框（写清要移动几个、到哪）后才动手 |
| **撤销本次归档** | 按刚才那次归档的日志原样移回 |
| **按日志撤销...** | 选任意一份 `.dedup-log-*.json`，撤销更早的某次归档 |
| 双击表格某行 | 弹出图片预览，确认"这两张是不是真的同一张" |
...
│   └── test_gui_dedup.py # 去重归档页的自动化测试(真开窗口点按钮, 需要图形显示)
├── requirements.txt
└── README.md
```

运行 `python samples/make_samples.py` 可重新生成示例图片。
运行 `python tests/test_dedup.py` 和 `python tests/test_gui_dedup.py` 可跑自动化测试（GUI 那个没有图形显示时会自动跳过）。
...
- **GUI 的去重归档页点了没反应？** 先点"开始预演"；没预演过时"一键归档"是灰的（这是故意的，防止误点直接搬文件）。
- **GUI 里怎么确认两张图真的一样？** 在清单里**双击任意一行**会弹出预览窗；基准行是灰色的，标着"基准(保留) → 归档文件夹名/"。
